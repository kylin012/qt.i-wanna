...
map2::map2(QWidget *parent) : QMainWindow(parent)
{
    //界面设置
       ...
       sound2 = new QSound(":/iwanna/warriors.wav",this);
       sound2->setLoops(-1);
       sound3 = new QSound(":/iwanna/promise.wav",this);
       sound3->setLoops(-1);
       //人物设置
       ...
       //背景图片初始化
       game[0] = new deadtool(":/iwanna/witcher1.png");
       game[1] = new deadtool(":/iwanna/witcher2.png");
       for(int i=0;i<2;i++){
           game[i]->setParent(this);
           game[i]->setGeometry(38,23,game[i]->width(),game[i]->height());
           game[i]->hide();
       }
       QLabel *mv = new QLabel(this);
       QMovie *movie = new QMovie(":/iwanna/nightmare1.gif");
       mv->setMovie(movie);
       mv->setScaledContents(true);
       mv->setGeometry(40,40,312,210);
       movie->start();
       movie->stop();
       mv->hide();
       QLabel *mv1 = new QLabel(this);
       QMovie *movie1 = new QMovie(":/iwanna/ani01.gif");
       mv1->setMovie(movie1);
       mv1->setScaledContents(true);
       mv1->setGeometry(0,0,this->width(),this->height());
       movie1->stop();
       mv1->hide();
       //文本读取
       QLineEdit *line = new QLineEdit("请输入您的名字",this);
       line->setSizeIncrement(100,30);
       line->move(900,300);
       line->hide();
       QPushButton *getsave = new QPushButton("查询",this);
       getsave->move(1000,300);
       getsave->hide();
       flag = true;
       connect(getsave,&QPushButton::clicked,[=](){
           QFile f("D:/QT Project/iwannnabe/save.txt");
           f.open(QIODevice::ReadOnly);
           QTextStream txtInput(&f);
           while(!txtInput.atEnd())
           {
               QString lineStr = txtInput.readLine();
               if(line->text()==lineStr){
                   QMessageBox::information(this,"您的信息",QString("您死亡了%1次，是位回头客！").arg(this->deadtimes));
                   flag=false;
                   break;
               }
           }
           f.close();
           if(flag){
               QMessageBox::information(this,"您的信息",QString("您死亡了%1次，是位萌新").arg(this->deadtimes));
...
void map2::keyPressEvent(QKeyEvent *ev){
    ...
    case Qt::Key_Z:
        ...
        connect(time1,&QTimer::timeout,[=](){
            if(bullet1->y>=24&&bullet1->y<=141&&bullet1->x<=160&&bullet1->x>=150){
                game[1]->hide();
                movable=true;
            }
